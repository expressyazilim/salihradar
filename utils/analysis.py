...
def local_z_score(matrix: np.ndarray, window_size: int = 15):
    # Lokal komşu piksel Z-Score hesaplayıcısı
...
def run_analysis_from_tiff_bytes(
    ...
    if smooth_on and smooth_k > 1:
        # V3 - Speckle Noise için Lee Filter
    ...
    def score_components(comps, sign_label):
        ...
        for comp in comps:
            ...
            if sign_label == "POS":
                if peak_abs > 4.5 and area > 20: smart_comment = "Güçlü anomali ancak çok geniş yayılım var. Metalik yapı veya mineralizasyon."
                elif peak_abs > 3.0 and area <= 15: smart_comment = "Kompakt ve çok güçlü hedef! Kesin donatı/metal veya yapı temeli şüphesi."
                elif fill < 0.3: smart_comment = "Parçalı pozitif yapı. Yanıltıcı uydu dağılımı olabilir, incelenmesi önerilir."
                else: smart_comment = "Standart yüksek kazançlı bölge. Yüzeyde potansiyel kütle."
            else:
                if peak_abs > 4.0 and area > 15: smart_comment = "Keskin negatif hacim! Derin boşluk, kazı veya tünel yapısı ihtimali."
                elif peak_abs > 2.5 and rel_z > 4: smart_comment = "Küçük ama sert düşüş. Gömülü ve izole derin bir boşluk yapısı olabilir."
                else: smart_comment = "Standart zemin altı negatif anomali / düşük yoğunluk."
